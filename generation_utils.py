...
def to_hdf(file_path, sim_params, data, sig_params, signal_len):
    ...
    if data[0]['H1'].shape[0]!=signal_len:
        print('WARNING: Signal of len {0} is being truncated to {1}'.format(data[0]['H1'].shape[0], signal_len))

    for i in range(0, len(data)):

        #Join signals as one Numpy array
        sig_set = convert_cbc_array_to_np(data[i], signal_len)
        ...
        time_set = convert_cbc_array_to_np([data[i]['H1'].sample_times, data[i]['L1'].sample_times,
                                            data[i]['V1'].sample_times], signal_len)
