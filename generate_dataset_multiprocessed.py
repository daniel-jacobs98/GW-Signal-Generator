...
def generate_and_inject_signal(param_set, injected_sig_list):
    signal = generate_signal(param_set)
    injected_signal = inject_signals_gaussian(signal, param_set['snr'], param_set)
    injected_sig_list.append((injected_signal, param_set))
...
if __name__ == '__main__':
    ...
    for i in range(0, sim_params['num_signals']):
        param_queue.put(next(get_param_set(sim_params)))
  
    #Make sure to save every ~1000 processes to prevent memory errors
    for ii in range(0, sim_params['num_signals']):
        manager = multiprocessing.Manager()
        injected_sig_list_with_params = manager.list()
        jobs = []
        p = multiprocessing.Process(target=generate_and_inject_signal, args=(param_queue.get(), injected_sig_list_with_params))
        p.start()
        jobs.append(p)

        if (ii%1000==0 and ii!=0) or ii==(sim_params['num_signals']-1):
            for proc in jobs:
                proc.join()
            jobs = []
            sig_list = [x[0] for x in injected_sig_list_with_params]
            param_list = [x[1] for x in injected_sig_list_with_params]
            injected_sig_list_with_params = manager.list()
            to_hdf(output_path, sim_params, sig_list, param_list, signal_len)
